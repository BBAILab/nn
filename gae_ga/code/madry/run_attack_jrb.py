...
def run_attack(checkpoint, x_adv, epsilon):  
  # replacement to use different source of MNIST iamges
  ...
  num_eval_examples = x_adv.shape[0]  # 10000  ; JRB substituted shape of numpy array for hard-coded number
  eval_batch_size = 64

  num_batches = int(math.ceil(num_eval_examples / eval_batch_size))
  total_corr = 0

  # x_nat = mnist.test.images  # JRB replaced with line below for evaluating adversarial examples based on training set
  # used alternate source of MNIST to match images with adverarial exampels
  # x_nat = mnist.train.images
  x_nat = train_images
  ...
  with tf.Session() as sess:
    ...
    for ibatch in range(num_batches):
      print('%s batch of %s batches' % (ibatch+1,num_batches))  # JRB added porint statement for progress monitoring
      bstart = ibatch * eval_batch_size
      bend = min(bstart + eval_batch_size, num_eval_examples)

      x_batch = x_adv[bstart:bend, :]
      # y_batch = mnist.test.labels[bstart:bend]  # replaced with line below for different MNIST data source
...
if __name__ == '__main__':
  ...
  # JRB commented out next two lines to permit evaluation of a greater number of adversarial examples
