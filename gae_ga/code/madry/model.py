...
class Model(object):
  def __init__(self):
    self.x_input = tf.compat.v1.placeholder(tf.float32, shape = [None, 784]) # JRB updated from tf.placeholder()
    self.y_input = tf.compat.v1.placeholder(tf.int64, shape = [None]) # JRB updated from tf.placeholder()
  ...
  def _weight_variable(shape):
      initial = tf.random.truncated_normal(shape, stddev=0.1) # JRB updated from tf.truncated_normal()
